...
class Seeker(models.Model):
    ...
    title = models.CharField('Title', max_length=50, null=False, blank=False)
    specialty = models.TextField('Specialty')
...
class ShiftPost(models.Model):
    posted_by = models.ForeignKey(Administrator, on_delete=models.CASCADE)
    facility = models.ForeignKey(Facility, on_delete=models.CASCADE)
    department = models.CharField('Department', max_length=50, null=False, blank=False)
    position = models.CharField('Position', max_length=50, null=False, blank=False)
    special_qualifications = models.TextField('Special Qualifications')
    start_date = models.DateField('Start Date', null=False, blank=False)
    end_date = models.DateField('End Date', null=False, blank=False)
    shift_start = models.TimeField('Shift Start', null=False, blank=False)
    shift_end = models.TimeField('Shift End', null=False, blank=False)
    rate_of_pay = models.CharField('Rate Of Pay', max_length=50, null=False, blank=False)